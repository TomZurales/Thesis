...
Before experimentation can take place on the SLAM system, the parameters of the VBEE library must be tuned to optimize performance. Parameters are split between the VBEE framework, and the observability model. The VBEE and model parameters are tightly coupled through feedback, making optimization difficult as the parameters of one system effects the other. As a solution, parameter optimization is performed in three phases. The first phase freezes the VBEE parameters to reasonable values while optimizing the observability model's parameters. The second phase freezes the observability model's parameters to the values obtained in phase 1 while optimizing the VBEE parameters. The final phase fine-tunes both parameter sets simultaneously, ideally achieving stable performance for the system.
