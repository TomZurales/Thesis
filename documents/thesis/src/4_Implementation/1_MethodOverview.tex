...
We define our icosahedral shell $\mathcal{S}$ as a construct of 20 faces $\mathcal{F}$. Each face carries parameters $d\in\mathbb{R}^+$ representing the maximum distance at which the point has been viewed through that face, and $p\in[0,1]$ representing the probability of seeing the point through that face.
\begin{*multline}
    abc
\end{*multline}
...
\begin{itemize}
    \item A vector of faces $\mathbf{F}$ of length 20\newline
          This is the metadata storage container
    \item A vector of face centers $\mathbf{F}_{center}$
\end{itemize}
