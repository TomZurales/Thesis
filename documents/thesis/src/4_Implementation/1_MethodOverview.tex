...
The observability model performs two functions: estimation of the probability of the map point being seen from a given viewpoint, and the integration of new observations into its internal representation. The estimation function takes a viewpoint as an argument, and returns the model's estimate of $P(S^{\boldsymbol{v}}|E)$. The update function takes an observation as an argument, and returns nothing. The goal of any model implementation is to minimize the objective function
$$
    \int |m(\boldsymbol{O},\boldsymbol{x},\boldsymbol{v}) - \operatorname{observable}(\boldsymbol{v})| \,d\theta\,d\phi\,dd
$$
