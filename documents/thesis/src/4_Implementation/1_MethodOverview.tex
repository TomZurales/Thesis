...
$$
    \phi(\mathbf{v}\in\mapthbb{S}^2) = \underset{j\in\{1..20\}}{\arg\max}(\mathbf{v} \cdot f_{i,j})
$$
where $f_{i,j}$ represents the unit vector pointing from $mp_i$ to the centroid of face $F_{i,j}$
