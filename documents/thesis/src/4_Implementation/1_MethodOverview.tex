...
It should be noted that within Euclidean space, a map point which is occluded from a viewpoint $\boldsymbol{v} = \{\theta, \phi, d\}$ will also be occluded from any viewpoint $\boldsymbol{v}' = \{\theta, \phi, d'\}$ where $d'>d$. Therefore, any positive observation from a direction which was previously observed to be occluded must be the result of a false positive, or be evedence 
