...
$$
where $\theta\in[0,2\pi)$ represents the azimuth, $\phi\in\left[-\frac{\pi}{2},\frac{\pi}{2}\right]$ represents the elevation, and $d\in[0,\infty)$ represents the distance between the map feature and the observer. Consider the simplified 2D observability plot shown in Figure \ref{fig:2d_observability}. The plot on the left represents the true observability of the point $p$, being observable from any angle and at any distance, except for angles and distances which place the observer behind the obstruction. However, generating the left plot in the real world would require infinite observations from infinite viewpoints
