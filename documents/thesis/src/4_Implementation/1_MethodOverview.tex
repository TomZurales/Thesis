...
The goal of the model is to estimate the global observability of a point using a finite set of $n$ observations $\boldsymbol{O} = \{o_0,\dots,o_n\}$, and a current viewpoint $\boldsymbol{v}$. Additionally, based on the model implementation, a set of parameters $\boldsymbol{x} = \{x_0,\dots,x_m\}
$$
...
A simple method for modeling the observability is the use of a K-nearest-neighbors clustering method. The parameters of interest in such a model would be $k$ and $n$, representing the number of neighbors and the number of samples stored in the model respectively. Such a model could be formulated as follows:
$$
