...
The generation of such a plot would require the generation and storage of infinite observations taken from across the domain of viewpoints. The objectives described in Section \ref{sec:objectives} state that the model's size should be constant, and kept as small as possible. Therefore, the models described below must operate either on some fixed, finite number of observations, or on a constant size set of metadata.

\subsubsection{Observability Modeling}

The purpose of the observability model is to estimate the global observability of a point using a finite set of $n$ observations $\boldsymbol{O} = \{o_0,\dots,o_n\}$, an implementation-dependent set of $k$ model parameters $\boldsymbol{x}=\{x_0,\dots,x_k\}$, and a current viewpoint $\boldsymbol{v}$.
$$
