...
It should be noted that within Euclidean space, a map point which is occluded along a vector ${\theta,\phi}$ at a distance $d$ will also be occluded along that vector at any distance $d'>d$. Therefore, any positive observation from a direction which was previously observed to be occluded must be the result of either a false positive, or the removal of the occlusion.
