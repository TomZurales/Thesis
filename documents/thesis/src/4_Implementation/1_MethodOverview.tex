...
$$
where $\boldsymbol{v} = \{\theta,\phi,d\}$  represents the azimuth, elevation, and distance between $p$ and the observe. This function returns 1 if the point is observable from the input viewpoint, and 0 if it is not. From this, an observation can be defined as a vector $o=\{\theta,\phi,v(\theta,\phi)\}\in[0,2\pi)\times\left[-\frac{\pi}{2},\frac{\pi}{2}\right]\times\{0,1\}$.
