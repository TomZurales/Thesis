...
The goal of the model is to provide the estimate the global observability of a point using a finite set of $n$ observations $\boldsymbol{O} = \{o_0,\dots,o_n\}$, and a current viewpoint $v=\{\theta,\phi,d\}$. Observations take the form $o_n=\{\theta_n,\phi_n,d_n,seen_n\}$, so the model can be written
$$
