...
\subsubsection{KNN Observability Model}
\label{sec:observability_models}

This thesis uses a K-nearest neighbors (KNN) method for modeling map point observability. The KNN model estimates $\psge$ as the ratio of positive observations within the $k$ nearest neighbors of the query viewpoint. Since infinite observations cannot be stored, the model maintains a finite buffer of $n$ observations. The angular distance between viewpoints is also considered, as distant observations may be irrelevant. Finally, the model incorporates feedback from the existence probability estimator to decide whether or not to integrate a new observation.
