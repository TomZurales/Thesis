...
The backend is responsible for integrating the coarse motion estimates and map points from the frontend into the larger context of the global map and trajectory, and optimizing this global estimate to minimize the total error. The backend assumes that the motion estimates and map points from the frontend have some error, meaning that simply appending new estimates from the frontend to the trajectory and map would quickly lead to drift. 