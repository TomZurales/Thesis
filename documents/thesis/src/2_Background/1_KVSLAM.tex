...
The backend is responsible for integrating the coarse motion estimates and map points from the frontend into the larger context of the global map and trajectory. Depending on the implementation, the backend may also perform several maintenance tasks designed to remove redundant data or handle various failure cases. The backend knows that there will be some error in the motion and map point estimations from the frontend, so it adjusts various elements of the global estimate such as frame positions, map point positions, and camera parameters to minimize the overall error within the system in an optimization process called bundle adjustment \cite{triggsBundleAdjustmentModern2000a}.

Additionally, some backend processes only run on an as-needed basis like loop closure, relocalization and map reuse.

Integration of the VD-OMR falls under the purview of the backend, as it is an extension to the existing point culling maintenance task. But this raises the question of why are map points culled in the first place?