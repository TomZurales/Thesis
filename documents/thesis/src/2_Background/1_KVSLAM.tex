...
The backend is responsible for integrating the coarse motion estimates and map points from the frontend into the larger context of the global map and trajectory. Depending on the implementation, the backend may also perform several maintenance tasks designed to remove redundant data or handle various failure cases.