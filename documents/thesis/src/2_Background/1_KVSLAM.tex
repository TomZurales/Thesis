...
Within this model, the VD-OMR system being researched would reside in the backend, specifically under the point culling subprocess. The purpose of point culling is to reduce the overall size of the map by removing map points which are redundant, outliers, or otherwise unhelpful. There are two key areas which benefit from reducing the number of points in the map to a minimal set of most useful, most correct data.