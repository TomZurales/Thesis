...
Loop closure is a technique used to correct drift during large scale KV-SLAM operations. The process works by recognizing when the camera enters a previously mapped area. Due to noise inherent to the system, the current position estimate may not match the position estimate from the previous vistiation.  Figure \ref{fig:loop_closure} shows how loop closure identifies a previously visited place, and corrects the trajectory to remove accumulated drift.
