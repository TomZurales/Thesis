...
Loop closure is a technique used to correct drift during large scale KV-SLAM operations. Loop closure recognize when the camera has entered a previously mapped area. Figure \ref{fig:loop_closure} shows how loop closure identifies a previously visited place, and corrects the trajectory to remove accumulated drift.
