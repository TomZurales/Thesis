...
As previously discussed, place recognition refers to the ability to realize when an agent has entered a previously visited area. In KV-SLAM, this is regularly accomplished using a bag-of-words approach on binary feature discriptors extracted from localized images \cite{camposORBSLAM3AccurateOpenSource2021}. However, the extracted features can change significantly over time due to lighting changes, moved objects, viewpoint, etc. Milford et al. improved robustness to time-of-day and weather variation by matching sequences of whole-images comparisons rather than relying on a single image or feature-based matches. However, this approach requires close agreement between the image sequence and the reference trajectory, essentially requiring that the camera be traveling at the same speed and in the same direction. Naseer et al. \cite{naseerVisionbasedMarkovLocalization2015} extended sequence-based place recognition by computing a full probability distribution over possible locations, allowing place recognition to succeeed without assuming trajectory parity. These approches are sensitive to viewpoint variation, performing poorly when the camera trajectory does not match previously explored trajectories.
