...
This research has two primary objectives. The first is to implement a novel map point removal method which integrates the historical viewpoint-dependent observability of map features into the determination of whether it has become outdated or is still relevant. The second is the integration of this map point removal method into an existing KV-SLAM system. This additional information may be relevant in other applications, but we focus on the specific use case of eliminating outdated map points from reused keypoint-based visual SLAM maps in stochastic SLAM operations. We can break this objective down into several prerequisites which must be completed before the objective can be called complete.
