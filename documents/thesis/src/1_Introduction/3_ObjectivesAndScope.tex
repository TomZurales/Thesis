...
The focus map reuse and the stochastic operating model can be motivated by the use cases discussed previously. On the ISS, the ideal navigation system would be a self-updating localizer than a SLAM system. The primary difference being that a localizer utilizes the same map and reference frame for all operations, while a SLAM system 
