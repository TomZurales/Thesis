...
The selection of visual SLAM was motivated primarily by the high performance to cost ratio of cameras when compared to other SLAM sensors. Both LiDAR and RGBD sensors perform direct 3D measurements of the environment, simplifying the overall SLAM process when compared to camera-based SLAM implementations which must estimate and refine 3D measurements over time. However, visual SLAM systems do perform exceptionally well under the right conditions, capable millimeter level localization accuracy. Additional engineering requirements may also force the use of visual SLAM, such as on the ISS where a LiDAR may interfere with sensitive equipemnt. Overall, for many applications
