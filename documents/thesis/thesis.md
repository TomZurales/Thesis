...
The field of research into making SLAM perform over long timeframes has been called lifetime SLAM[], eluding to the fact that SLAM systems with the requirement for an unchanging environment will still be able to operate successfully over short timeframes, but will struggle with missions which take place over multiple days, weeks, months, or years. Numerous methods for improving performance over time have been implemented, pushing the field of SLAM to the point where it is now seeing deployment in numerous dynamic environments. We will discuss these implementations in the Background section, with a focus on their strengths, weaknesses, and overall effectiveness. A subset of lifetime SLAM covers the use of pre-existing maps, rather than always maintaining the active map. This approach 
